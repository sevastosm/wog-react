...
export default function ({ listItem }) {
  ...
  const { activeVideo, setVideo, lang } = useContext(AppContext);
  ...
  const handleClick = () => {
    ...
    history.push(`/${lang}/${listItem.SeriesID}/${listItem.YouTubeId}`);
    setVideo(listItem);
  };
  ...
  return (
    <div
      ...
      className={`listItem mx-3 ${
        listItem?.RecordingID === activeVideo?.RecordingID ? "active" : ""
      }`}
      key={listItem?.RecordingID}
    >
